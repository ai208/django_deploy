...
class HealthRecordCreateView(PermissionRequiredMixin,LoginRequiredMixin,CreateView):
    model = models.HealthRecord
    template_name = 'healthapp/healthrecord_create.html'
    form_class = HealthRecordForm
    success_url = reverse_lazy('healthrecord_list')
    permission_required = 'healthapp.add_healthrecord'
    def form_valid(self, form):
        form.instance.user = self.request.user
        return super().form_valid(form)

class HealthRecordUpdateView(UpdateView,PermissionRequiredMixin,LoginRequiredMixin):
    model = models.HealthRecord
    ...
    def get_queryset(self):
        return models.HealthRecord.objects.filter(user=self.request.user)
class HealthRecordDeleteView(DeleteView,PermissionRequiredMixin,LoginRequiredMixin):
    model = models.HealthRecord
